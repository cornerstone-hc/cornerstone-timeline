...
# With line colours varying
d$fromcol <- 1993
d$tocol <- 2015

cols <- rep(c("green", "blue"), length.out = nrow(d))

ggplot(d)  +
  geom_segment(aes(x = fromcol, xend = tocol, y = Name, yend = Name, colour = cols ), size = 1) +  geom_segment(aes(x = From, xend = Until, y = Name, yend = Name ), size = 2) +
  theme(axis.title = element_blank()) + theme_chc
